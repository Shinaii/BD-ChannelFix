 ...
 */
 ...
 module.exports = (_ => {
	...
	return !window.Fix_BDFDB_Global || (!window.Fix_BDFDB_Global.loaded && !window.Fix_BDFDB_Global.started) ? class {
		...
		load () {
			if (!window.Fix_BDFDB_Global || !Array.isArray(window.Fix_BDFDB_Global.pluginQueue)) window.Fix_BDFDB_Global = Object.assign({}, window.Fix_BDFDB_Global, {pluginQueue: []});
			if (!window.Fix_BDFDB_Global.downloadModal) {
				window.Fix_BDFDB_Global.downloadModal = true;
				BdApi.showConfirmationModal("Library Missing", `The Library Plugin needed for ${config.info.name} is missing. Please click "Download Now" to install it.`, {
					confirmText: "Download Now",
					cancelText: "Cancel",
					onCancel: _ => {delete window.Fix_BDFDB_Global.downloadModal;},
					onConfirm: _ => {
						delete window.Fix_BDFDB_Global.downloadModal;
						this.downloadLibrary();
					}
				});
			}
			if (!window.Fix_BDFDB_Global.pluginQueue.includes(config.info.name)) window.Fix_BDFDB_Global.pluginQueue.push(config.info.name);
		}
		start () {this.load();}
		stop () {}
		getSettingsPanel () {
		...
		const UserRowComponent = class UserRow extends BdApi.React.Component {
			componentDidMount() {
				if (this.props.user.fetchable) {
					...
					Fix_BDFDB.LibraryModules.UserProfileUtils.getUser(this.props.user.id).then(fetchedUser => {
						this.props.user = Object.assign({}, fetchedUser, Fix_BDFDB.LibraryModules.MemberStore.getMember(this.props.guildId, this.props.user.id) || {});
						Fix_BDFDB.ReactUtils.forceUpdate(this);
					});
				}
			}
			render() {
				return Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.ListRow, {
					prefix: Fix_BDFDB.ReactUtils.createElement("div", {
						className: Fix_BDFDB.disCN.listavatar,
						children: Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.AvatarComponents.default, {
							src: Fix_BDFDB.UserUtils.getAvatar(this.props.user.id),
							status: Fix_BDFDB.UserUtils.getStatus(this.props.user.id),
							size: Fix_BDFDB.LibraryComponents.AvatarComponents.Sizes.SIZE_40,
							onClick: _ => {
								if (accessModal) accessModal.props.onClose();
								Fix_BDFDB.LibraryModules.UserProfileModalUtils.openUserProfileModal({
					...
					labelClassName: Fix_BDFDB.disCN.nametag,
					label: [
						Fix_BDFDB.ReactUtils.createElement("span", {
							className: Fix_BDFDB.disCN.username,
							children: this.props.user.nick || this.props.user.username,
							style: {color: this.props.user.colorString}
						}),
						!this.props.user.discriminator ? null : Fix_BDFDB.ReactUtils.createElement("span", {
							className: Fix_BDFDB.disCN.listdiscriminator,
							children: `#${this.props.user.discriminator}`
						}),
						this.props.user.bot && Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.BotTag, {
							style: {marginLeft: 6}
						})
					]
				});
		...
		const RoleRowComponent = class RoleRow extends BdApi.React.Component {
			render() {
				return Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.ListRow, {
					prefix: Fix_BDFDB.ReactUtils.createElement("div", {
						className: Fix_BDFDB.disCNS.avataricon + Fix_BDFDB.disCNS.listavatar + Fix_BDFDB.disCNS.avatariconsizemedium + Fix_BDFDB.disCN.avatariconinactive,
						style: {
						...
						children: Fix_BDFDB.ReactUtils.createElement("div", {
							style: {
								...
								height: "100%",
								width: "100%",
								backgroundColor: Fix_BDFDB.ColorUtils.convert(this.props.role.colorString, "RGB") || Fix_BDFDB.DiscordConstants.Colors.PRIMARY_DARK_300
							}
						})
					}),
					labelClassName: this.props.role.overwritten && Fix_BDFDB.disCN.strikethrough,
					label: Fix_BDFDB.ReactUtils.createElement("span", {
		...
		return class ShowHiddenChannels extends Plugin {
			onLoad () {
				overrideTypes = Object.keys(Fix_BDFDB.DiscordConstants.PermissionOverrideType);
				...
				this.css = `
					${Fix_BDFDB.dotCNS._showhiddenchannelsaccessmodal + Fix_BDFDB.dotCN.messagespopoutemptyplaceholder} {
						position: absolute;
						bottom: 0;
						width: 100%;
					}
			...
			onStart () {
				...
				Fix_BDFDB.PatchUtils.patch(this, Fix_BDFDB.LibraryModules.GuildUtils, "setChannel", {instead: e => {
					let channelId = (Fix_BDFDB.LibraryModules.VoiceUtils.getVoiceStateForUser(e.methodArguments[1]) || {}).channelId;
					if (!channelId || !this.isChannelHidden(channelId)) return e.callOriginalMethod();
				}});
				
				Fix_BDFDB.PatchUtils.patch(this, Fix_BDFDB.LibraryModules.UnreadChannelUtils, "hasUnread", {after: e => {
					return e.returnValue && !this.isChannelHidden(e.methodArguments[0]);
				}});
				
				Fix_BDFDB.PatchUtils.patch(this, Fix_BDFDB.LibraryModules.UnreadChannelUtils, "getMentionCount", {after: e => {
			...
			getSettingsPanel (collapseStates = {}) {
				let settingsPanel;
				return settingsPanel = Fix_BDFDB.PluginUtils.createSettingsPanel(this, {
					...
					children: _ => {
						...
						settingsItems.push(Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.CollapseContainer, {
							title: "Settings",
							collapseStates: collapseStates,
							children: Object.keys(this.defaults.sortOrder).map(key => Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.SettingsSaveItem, {
								type: "Select",
								plugin: this,
								keys: ["sortOrder", key],
							...
							})).concat(Object.keys(this.defaults.general).map(key => Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.SettingsSaveItem, {
								type: "Switch",
								plugin: this,
								keys: ["general", key],
								label: this.defaults.general[key].description,
								value: this.settings.general[key]
							}))).concat(Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.SettingsPanelList, {
								title: "Show Channels:",
								children: Object.keys(this.defaults.channels).map(key => Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.SettingsSaveItem, {
									type: "Switch",
									plugin: this,
									keys: ["channels", key],
									label: Fix_BDFDB.LanguageUtils.LanguageStrings[typeNameMap[key]],
						...
						settingsItems.push(Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.CollapseContainer, {
							...
							collapseStates: collapseStates,
							children: [
								Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.SettingsGuildList, {
									className: Fix_BDFDB.disCN.marginbottom20,
									disabled: blackList,
									onClick: disabledGuilds => this.saveBlackList(disabledGuilds)
								}),
								Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.SettingsItem, {
									type: "Button",
									color: Fix_BDFDB.LibraryComponents.Button.Colors.GREEN,
									label: "Enable for all Servers",
									onClick: _ => this.batchSetGuilds(settingsPanel, collapseStates, true),
									children: Fix_BDFDB.LanguageUtils.LanguageStrings.ENABLE
								}),
								Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.SettingsItem, {
									type: "Button",
									color: Fix_BDFDB.LibraryComponents.Button.Colors.PRIMARY,
									label: "Disable for all Servers",
									onClick: _ => this.batchSetGuilds(settingsPanel, collapseStates, false),
									children: Fix_BDFDB.LanguageUtils.LanguageStrings.DISABLE
								})
							]
						}));
			...
			forceUpdateAll () {				
				...
				Fix_BDFDB.PatchUtils.forceAllUpdates(this);
				Fix_BDFDB.ChannelUtils.rerenderAll();
			}
		
			onUserContextMenu (e) {
				if (e.subType == "useUserManagementItems" || e.subType == "useMoveUserVoiceItems" || e.subType == "usePreviewVideoItem") {
					let channelId = (Fix_BDFDB.LibraryModules.VoiceUtils.getVoiceStateForUser(e.instance.props.user.id) || {}).channelId;
			...
			onChannelContextMenu (e) {
				if (e.instance.props.channel && e.instance.props.channel.guild_id && e.subType == "useChannelMarkAsReadItem") {
					...
					if (isHidden || this.settings.general.showForNormal) {
						if (e.returnvalue.length) e.returnvalue.push(Fix_BDFDB.ContextMenuUtils.createItem(Fix_BDFDB.LibraryComponents.MenuItems.MenuSeparator, {}));
						e.returnvalue.push(Fix_BDFDB.ContextMenuUtils.createItem(Fix_BDFDB.LibraryComponents.MenuItems.MenuItem, {
							label: this.labels.context_channelaccess,
							id: Fix_BDFDB.ContextMenuUtils.createItemId(this.name, "permissions"),
			...
			onGuildContextMenu (e) {
				if (e.instance.props.guild) {
					let [children, index] = Fix_BDFDB.ContextMenuUtils.findItem(e.returnvalue, {id: "hide-muted-channels"});
					if (index > -1) children.splice(index + 1, 0, Fix_BDFDB.ContextMenuUtils.createItem(Fix_BDFDB.LibraryComponents.MenuItems.MenuCheckboxItem, {
						label: this.labels.context_hidehidden,
						id: Fix_BDFDB.ContextMenuUtils.createItemId(this.name, "hide-locked-channels"),
						checked: blackList.includes(e.instance.props.guild.id),
						action: value => {
							if (value) blackList.push(e.instance.props.guild.id);
							else Fix_BDFDB.ArrayUtils.remove(blackList, e.instance.props.guild.id, true);
							this.saveBlackList(Fix_BDFDB.ArrayUtils.removeCopies(blackList));

							Fix_BDFDB.PatchUtils.forceAllUpdates(this);
							Fix_BDFDB.ChannelUtils.rerenderAll(true);
						}
					}));
				}
			}
			...
			processChannels (e) {
				...
				let processCategory = (category, insertChannelless) => {
					if (!category) return;
					let channelArray = Fix_BDFDB.ObjectUtils.toArray(category.channels);
					if (channelArray.length) {
						for (let n of channelArray) if ((n.renderLevel == renderLevels.CAN_NOT_SHOW || n._hidden) && e.instance.props.selectedVoiceChannelId != n.record.id) {
							if (show && (this.settings.channels[Fix_BDFDB.DiscordConstants.ChannelTypes[n.record.type]] || this.settings.channels[Fix_BDFDB.DiscordConstants.ChannelTypes[n.record.type]] === undefined)) {
								n._hidden = true;
						...
						category.shouldShowEmptyCategory = Fix_BDFDB.TimeUtils.suppress((...args) => {
							if (!this.started) {
			...
			processChannelItem (e) {
				if (e.instance.props.channel && this.isChannelHidden(e.instance.props.channel.id)) {
					if (!e.returnvalue) e.instance.props.className = Fix_BDFDB.DOMUtils.formatClassName(e.instance.props.className, Fix_BDFDB.disCN._showhiddenchannelshiddenchannel);
					else {
						let [children, index] = Fix_BDFDB.ReactUtils.findParent(e.returnvalue, {name: "ChannelItemIcon"});
						let channelChildren = Fix_BDFDB.ReactUtils.findChild(e.returnvalue, {props: [["className", Fix_BDFDB.disCN.channelchildren]]});
						if (channelChildren && channelChildren.props && channelChildren.props.children) {
							channelChildren.props.children = [Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.TooltipContainer, {
								text: Fix_BDFDB.LanguageUtils.LanguageStrings.CHANNEL_LOCKED_SHORT,
								children: Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.Clickable, {
									className: Fix_BDFDB.disCN.channeliconitem,
									style: {display: "block"},
									children: Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.SvgIcon, {
										className: Fix_BDFDB.disCN.channelactionicon,
										name: Fix_BDFDB.LibraryComponents.SvgIcon.Names.LOCK_CLOSED
									})
								})
							})];
						}
						if (!(e.instance.props.channel.type == Fix_BDFDB.DiscordConstants.ChannelTypes.GUILD_VOICE && e.instance.props.connected)) {
							let wrapper = Fix_BDFDB.ReactUtils.findChild(e.returnvalue, {props: [["className", Fix_BDFDB.disCN.channelwrapper]]});
							if (wrapper) {
								wrapper.props.onMouseDown = event => Fix_BDFDB.ListenerUtils.stopEvent(event);
								wrapper.props.onMouseUp = event => Fix_BDFDB.ListenerUtils.stopEvent(event);
							}
							let mainContent = Fix_BDFDB.ReactUtils.findChild(e.returnvalue, {props: [["className", Fix_BDFDB.disCN.channelmaincontent]]});
							if (mainContent) {
								mainContent.props.onClick = event => Fix_BDFDB.ListenerUtils.stopEvent(event);
			...
			isChannelHidden (channelId) {
				let channel = Fix_BDFDB.LibraryModules.ChannelStore.getChannel(channelId);
			...
			batchSetGuilds (settingsPanel, collapseStates, value) {
				if (!value) {
					for (let id of Fix_BDFDB.LibraryModules.FolderStore.getFlattenedGuildIds()) blackList.push(id);
					this.saveBlackList(Fix_BDFDB.ArrayUtils.removeCopies(blackList));
				}
				else this.saveBlackList([]);
				Fix_BDFDB.PluginUtils.refreshSettingsPanel(this, settingsPanel, collapseStates);
			}

			getBlackList () {
				let loadedBlackList = Fix_BDFDB.DataUtils.load(this, "blacklist");
				return !Fix_BDFDB.ArrayUtils.is(loadedBlackList) ? [] : loadedBlackList;
			
			}
			
			saveBlackList (savedBlackList) {
				blackList = savedBlackList;
				Fix_BDFDB.DataUtils.save(savedBlackList, this, "blacklist");
			}
			
			openAccessModal (channel, allowed) {
				let isThread = Fix_BDFDB.ChannelUtils.isThread(channel);
				let guild = Fix_BDFDB.LibraryModules.GuildStore.getGuild(channel.guild_id);
				let myMember = guild && Fix_BDFDB.LibraryModules.MemberStore.getMember(guild.id, Fix_BDFDB.UserUtils.me.id);
				
				let parentChannel = isThread && Fix_BDFDB.LibraryModules.ChannelStore.getChannel(Fix_BDFDB.LibraryModules.ChannelStore.getChannel(channel.id).parent_id);
				let category = parentChannel && parentChannel.parent_id && Fix_BDFDB.LibraryModules.ChannelStore.getChannel(parentChannel.parent_id) || Fix_BDFDB.LibraryModules.ChannelStore.getChannel(Fix_BDFDB.LibraryModules.ChannelStore.getChannel(channel.id).parent_id);
				
				let lightTheme = Fix_BDFDB.DiscordUtils.getTheme() == Fix_BDFDB.disCN.themelight;
				
				let addUser = (id, users) => {
					let user = Fix_BDFDB.LibraryModules.UserStore.getUser(id);
					if (user) users.push(Object.assign({}, user, Fix_BDFDB.LibraryModules.MemberStore.getMember(guild.id, id) || {}));
					else users.push({id: id, username: `UserId: ${id}`, fetchable: true});
				};
				let checkAllowPerm = permString => {
					return (permString | Fix_BDFDB.DiscordConstants.Permissions.VIEW_CHANNEL) == permString && (channel.type != Fix_BDFDB.DiscordConstants.ChannelTypes.GUILD_VOICE || (permString | Fix_BDFDB.DiscordConstants.Permissions.CONNECT) == permString);
				};
				...
				for (let id in channel.permissionOverwrites) {
					if ((channel.permissionOverwrites[id].type == Fix_BDFDB.DiscordConstants.PermissionOverrideType.ROLE || overrideTypes[channel.permissionOverwrites[id].type] == Fix_BDFDB.DiscordConstants.PermissionOverrideType.ROLE) && (guild.roles[id] && guild.roles[id].name != "@everyone") && checkAllowPerm(channel.permissionOverwrites[id].allow)) {
						allowedRoles.push(Object.assign({overwritten: myMember && myMember.roles.includes(id) && !allowed}, guild.roles[id]));
					}
					else if ((channel.permissionOverwrites[id].type == Fix_BDFDB.DiscordConstants.PermissionOverrideType.MEMBER || overrideTypes[channel.permissionOverwrites[id].type] == Fix_BDFDB.DiscordConstants.PermissionOverrideType.MEMBER) && checkAllowPerm(channel.permissionOverwrites[id].allow)) {
						addUser(id, allowedUsers);
					}
					if ((channel.permissionOverwrites[id].type == Fix_BDFDB.DiscordConstants.PermissionOverrideType.ROLE || overrideTypes[channel.permissionOverwrites[id].type] == Fix_BDFDB.DiscordConstants.PermissionOverrideType.ROLE) && checkDenyPerm(channel.permissionOverwrites[id].deny)) {
						deniedRoles.push(guild.roles[id]);
						if (guild.roles[id] && guild.roles[id].name == "@everyone") everyoneDenied = true;
					}
					else if ((channel.permissionOverwrites[id].type == Fix_BDFDB.DiscordConstants.PermissionOverrideType.MEMBER || overrideTypes[channel.permissionOverwrites[id].type] == Fix_BDFDB.DiscordConstants.PermissionOverrideType.MEMBER) && checkDenyPerm(channel.permissionOverwrites[id].deny)) {
				...
				for (let id in guild.roles) if ((guild.roles[id].permissions | Fix_BDFDB.DiscordConstants.Permissions.ADMINISTRATOR) == guild.roles[id].permissions && ![].concat(allowedRoles, deniedRoles).find(role => role.id == id)) allowedRoles.push(Object.assign({overwritten: myMember && myMember.roles.includes(id) && !allowed}, guild.roles[id]));
				if (allowed && !everyoneDenied) allowedRoles.push({name: "@everyone"});
				
				let allowedElements = [], deniedElements = [];
				for (let role of allowedRoles) allowedElements.push(Fix_BDFDB.ReactUtils.createElement(RoleRowComponent, {role: role, guildId: guild.id, channelId: channel.id}));
				for (let user of allowedUsers) allowedElements.push(Fix_BDFDB.ReactUtils.createElement(UserRowComponent, {user: user, guildId: guild.id, channelId: channel.id}));
				for (let role of deniedRoles) deniedElements.push(Fix_BDFDB.ReactUtils.createElement(RoleRowComponent, {role: role, guildId: guild.id, channelId: channel.id}));
				for (let user of deniedUsers) deniedElements.push(Fix_BDFDB.ReactUtils.createElement(UserRowComponent, {user: user, guildId: guild.id, channelId: channel.id}));
				
				const infoStrings = [
					isThread && {
						title: Fix_BDFDB.LanguageUtils.LanguageStrings.THREAD_NAME,
						text: channel.name
					}, !isThread && {
						title: Fix_BDFDB.LanguageUtils.LanguageStrings.FORM_LABEL_CHANNEL_NAME,
						text: channel.name
					}, channel.type == Fix_BDFDB.DiscordConstants.ChannelTypes.GUILD_VOICE ? {
						title: Fix_BDFDB.LanguageUtils.LanguageStrings.FORM_LABEL_BITRATE,
						text: channel.bitrate || "---"
					} : {
						title: Fix_BDFDB.LanguageUtils.LanguageStrings.FORM_LABEL_CHANNEL_TOPIC,
						text: Fix_BDFDB.ReactUtils.markdownParse(channel.topic || "---")
					}, {
						title: Fix_BDFDB.LanguageUtils.LanguageStrings.CHANNEL_TYPE,
						text: Fix_BDFDB.LanguageUtils.LanguageStrings[typeNameMap[Fix_BDFDB.DiscordConstants.ChannelTypes[channel.type]]]
					}, isThread && parentChannel && {
						title: Fix_BDFDB.LanguageUtils.LanguageStrings.FORM_LABEL_CHANNEL_NAME,
						text: parentChannel.name
					}, {
						title: Fix_BDFDB.LanguageUtils.LanguageStrings.CATEGORY_NAME,
						text: category && category.name || Fix_BDFDB.LanguageUtils.LanguageStrings.NO_CATEGORY
					}
				].map((formLabel, i) => formLabel && [
					i == 0 ? null : Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.FormComponents.FormDivider, {
						className: Fix_BDFDB.disCN.marginbottom20
					}),
					Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.FormComponents.FormItem, {
						title: `${formLabel.title}:`,
						className: Fix_BDFDB.DOMUtils.formatClassName(Fix_BDFDB.disCN.marginbottom20, i == 0 && Fix_BDFDB.disCN.margintop8),
						children: Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.FormComponents.FormText, {
							className: Fix_BDFDB.disCN.marginleft8,
							children: formLabel.text
						})
					})
				]).flat(10).filter(n => n);

				Fix_BDFDB.ModalUtils.open(this, {
					size: "MEDIUM",
					header: Fix_BDFDB.LanguageUtils.LanguageStrings.CHANNEL + " " + Fix_BDFDB.LanguageUtils.LanguageStrings.ACCESSIBILITY,
					subHeader: "#" + channel.name,
					className: Fix_BDFDB.disCN._showhiddenchannelsaccessmodal,
					contentClassName: Fix_BDFDB.DOMUtils.formatClassName(!isThread && Fix_BDFDB.disCN.listscroller),
					onOpen: modalInstance => {if (modalInstance) accessModal = modalInstance;},
					children: isThread ? infoStrings : [
						Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.ModalComponents.ModalTabContent, {
							className: Fix_BDFDB.disCN.modalsubinner,
							tab: Fix_BDFDB.LanguageUtils.LanguageStrings.OVERLAY_SETTINGS_GENERAL_TAB,
							children: infoStrings
						}),
						Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.ModalComponents.ModalTabContent, {
							tab: this.labels.modal_allowed,
							children: allowedElements.length ? allowedElements :
								Fix_BDFDB.ReactUtils.createElement(Fix_BDFDB.LibraryComponents.MessagesPopoutComponents.EmptyStateBottom, {
									msg: Fix_BDFDB.LanguageUtils.LanguageStrings.AUTOCOMPLETE_NO_RESULTS_HEADER,
			...
			setLabelsByLanguage () {
				switch (Fix_BDFDB.LanguageUtils.getLanguage().id) {
					case "bg":		// Bulgarian
						return {
	...
	})(window.Fix_BDFDB_Global.PluginUtils.buildPlugin(config));
})();